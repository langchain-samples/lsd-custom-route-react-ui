...
import { APP_DESCRIPTION, APP_NAME, FRONTEND_ENV_FILE } from "./constants";
...
function AppHeader({
  ...
  return (
    <header className="header-blur sticky top-0 z-30 flex items-center justify-between border-b border-[var(--border)] px-6 py-3">
      <div className="flex items-center gap-3">
        <svg className="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="1.5">
          <path d="M12 2L2 12l10 10 10-10L12 2z" />
          <path d="M12 8L8 12l4 4 4-4-4-4z" />
        </svg>
        <div>
          <h1 className="text-lg font-semibold">{APP_NAME}</h1>
          <p className="text-xs text-[var(--muted-foreground)]">{APP_DESCRIPTION}</p>
        </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-lg border border-[var(--border)] px-3 py-1.5 text-xs font-medium text-[var(--muted-foreground)] transition-colors hover:bg-[var(--accent-bg)]"
        >
          <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M5 12h14" />
            <path d="M12 5v14" />
...
function AuthenticatedApp({ session }: { session: Session }) {
  ...
  return (
    <div className="flex h-dvh flex-col bg-[var(--background)]">
      ...
      <footer className="bg-[var(--background)] px-4 py-4">
        <form
          onSubmit={(event) => {
...
export default function App() {
  ...
  if (supabaseConfigError) {
    return (
      <ConfigNotice
        title="Missing frontend configuration"
        message={`Copy \`${FRONTEND_ENV_FILE}.example\` to \`${FRONTEND_ENV_FILE}\`, then add the Supabase browser credentials before loading the app.`}
        details={[
